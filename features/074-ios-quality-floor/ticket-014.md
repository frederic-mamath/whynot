...
The recent palette migration (feature 072) replaced hex codes with `Colors.*`. The audit found ~148 hardcoded `fontSize`, plus ~25 files with hardcoded `padding`/`borderRadius` literals. R9 was added in T-003 with these files seeded as TODO exclusions.
...
After this ticket, `ios-app/CLAUDE.md`'s "Styling" section loses its prose paragraph about hardcoded values — R9 enforces it.

## Acceptance Criteria

- As a user, the visual rhythm of spacing, padding, and font sizes is uniform across every screen
- As a developer, the `R9_EXCLUDE` set in `scripts/arch-test.mjs` is empty (only the always-excluded `src/theme/tokens.ts` remains via the dedicated check)
- As a developer, `npm run arch:test` passes with R9 enforced everywhere
...
- Frontend / Sweep
  - Migrate each file in `R9_EXCLUDE` and remove its entry. Mechanical replacement using the mapping above.
...
- CLAUDE.md update
  - `ios-app/CLAUDE.md` — trim "Styling" section's prose about hardcoded values; reference R9 instead
...
Output ends with `Architecture checks passed (R2, R3, R4, R7, R8, R9).` and the `R9_EXCLUDE` set in `scripts/arch-test.mjs` is empty. Visual diff each migrated screen against pre-migration screenshots — no shifts.
