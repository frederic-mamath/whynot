...
- Backend (`app/src/`)
  - Package
    - `app/package.json`
      - Install `codes-postaux` (French postcode → centroid lookup, offline, ~700KB)
  - Service
    - `app/src/services/MondialRelayService.ts`
      ...
      - In `searchRelayPoints`: look up postcode centroid via `codes-postaux`, then for each relay point compute `distanceKm` rounded to 1 decimal. If centroid lookup fails, set `distanceKm: null` for all points.
  - Router
...
None — the `codes-postaux` package ships postcode data offline; no API keys required.
