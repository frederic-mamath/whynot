...
WhyNot uses Docker Compose for local development with three services:

```yaml
services:
  backend: # Node.js API + React frontend
  postgres: # PostgreSQL database
  redis: # Redis cache & job queue
```

**Network**: `whynot-network` (bridge) - all services communicate via internal DNS
...
- PostgreSQL: `localhost:54321` → container `5432` (custom port to avoid conflicts)
- Redis: `localhost:6379` → container `6379`

See **[DOCKER.md](DOCKER.md)** for detailed Docker development guide.
...
- **Web Service**: whynot-backend (Docker, Free/Starter plan)
- **PostgreSQL**: Render managed database (Free 1GB)
...
4. Runs health check (/health endpoint)
5. Migrations run automatically on startup (docker-entrypoint.sh)
...
|----------|-------------|-------|
| Render (Free) | $0 | 15min sleep after inactivity |
| Render (Starter) | $7 | No sleep, always responsive |
| Heroku (Legacy) | $815 | Previous platform (99% more expensive) |
...
```bash
DB_HOST=<auto-from-render>  # Render PostgreSQL host
DB_PORT=5432
DB_NAME=<auto-from-render>
DB_USER=<auto-from-render>
DB_PASSWORD=<auto-from-render>
REDIS_URL=<from-upstash>    # Upstash Redis connection string
NODE_ENV=production
JWT_SECRET=<auto-generated>
AGORA_APP_ID=<manual>
AGORA_APP_CERTIFICATE=<manual>
CLOUDFLARE_STREAM_ACCOUNT_ID=<manual>
CLOUDFLARE_STREAM_API_TOKEN=<manual>
STRIPE_SECRET_KEY=<manual>
STRIPE_WEBHOOK_SECRET=<manual>
```

See **[RENDER_ENV_CHECKLIST.md](RENDER_ENV_CHECKLIST.md)** for complete environment variable guide.
...
Stage 2 (builder): Build backend (tsc) + frontend (vite)
Stage 3 (runner):  Production dependencies + built artifacts
```
...
- Returns: `{"status":"healthy","uptime":123.45,"environment":"production"}`
- Used by Render to verify deployment success

**Frontend Serving**: Backend serves built React app from `/dist/public`
