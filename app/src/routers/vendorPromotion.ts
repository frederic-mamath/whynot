    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!isAssociated) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "Product must be associated with the channel before promotion",
        });
      }

      // Check if already promoting
      const alreadyPromoting =
        await vendorPromotedProductRepository.isPromoting(
          input.channelId,
          ctx.user.id,
          input.productId,
        );
    ...
    .query(async ({ ctx, input }) => {
      ...
      const promotions = await db
        ...
        .innerJoin(
          "products",
          "products.id",
          "vendor_promoted_products.product_id",
        )
      ...
      const products = await db
        ...
        .leftJoin(
          (eb) =>
            eb
              .selectFrom("vendor_promoted_products")
              .select(["product_id", "vendor_id", "unpromoted_at"])
