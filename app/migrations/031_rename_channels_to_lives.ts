...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`ALTER TABLE channel_participants RENAME TO live_participants`.execute(db);
  ...
  await sql`ALTER TABLE live_participants RENAME COLUMN channel_id TO live_id`.execute(db);
  await sql`ALTER TABLE live_products RENAME COLUMN channel_id TO live_id`.execute(db);
  ...
  await sql`ALTER TABLE lives ADD COLUMN session_stopped_at TIMESTAMP`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`ALTER TABLE lives DROP COLUMN IF EXISTS session_stopped_at`.execute(db);
  ...
  // Rename columns back
  await sql`ALTER TABLE live_participants RENAME COLUMN live_id TO channel_id`.execute(db);
  await sql`ALTER TABLE live_products RENAME COLUMN live_id TO channel_id`.execute(db);

  // Rename tables back
  await sql`ALTER TABLE live_products RENAME TO channel_products`.execute(db);
  await sql`ALTER TABLE live_participants RENAME TO channel_participants`.execute(db);
